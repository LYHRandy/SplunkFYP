...
SCHEME = """<scheme>
    <title>LTA(Traffic)</title>
    <description>Get data from LTA</description>
...
def lta_connection(accountKey,uniqueUserID):
        url="http://datamall.mytransport.sg/ltaodataservice.svc/IncidentSet"
...
"""def validate_config(accountKey,uniqueUserID):
        try:
                ...
                result = urllib2.urlopen(request)
                if resp.status != 200:
                        raise Exception,"HTTP request to LTA data returned with status code %d" % (result.getcode())
...
def run():
        ...
        while True:
                lta_connection(AccountKey,UniqueUserID)
                #date_time()
                time.sleep(300)
